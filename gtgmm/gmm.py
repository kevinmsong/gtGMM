...
class GMMOptimizer:
    ...
    def extract_gene_components(self, terrain):
        """
        Extract gene-to-component mappings from GMM results.
        
        Parameters
        ----------
        terrain : GeneTerrain
            The terrain object used for GMM analysis
            
        Returns
        -------
        dict
            Dictionary mapping component indices to DataFrames with gene information
        """
        if self.labels is None:
            raise ValueError("GMM has not been fitted yet")
        
        components = {}
        n_components = len(set(self.labels))
        
        for comp_idx in range(n_components):
            comp_genes = []
            for _, row in terrain.gene_data.iterrows():
                gene = row['gene']
                x = int(row['X'] * terrain.resolution)
                y = int(row['Y'] * terrain.resolution)
                
                if 0 <= x < terrain.resolution and 0 <= y < terrain.resolution:
                    idx = y * terrain.resolution + x
                    if idx < len(self.labels) and self.labels[idx] == comp_idx:
                        comp_genes.append(gene)
            
            components[comp_idx] = pd.DataFrame({'gene': comp_genes})
